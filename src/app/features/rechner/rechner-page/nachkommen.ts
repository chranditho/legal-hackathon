  ...
  template: `
    <section>
      ...
      <div class="persons-container">
        ...
        <div class="person-box" *ngFor="let person of filteredPersonenListe; let i = index">
            <div *ngIf="person.Art.name &&  (person.Art.name.toLowerCase().includes('geschwister') || person.Art.name.toLowerCase().includes('kinder'))">
              <div class="person-name">{{person.Art.name || person.Art.PersonenArt}}</div>
              <div class="person-type">{{person.Art.PersonenArt}}</div>
              <div class="person-exclusion-options">
                  <p *ngIf="!person.Art.name.toLowerCase().includes('cousin') || person.Art.name.toLowerCase().includes('kinder')">      
                  <mat-button-toggle-group (change)="onToggleChange($event, person.Art.name)">
                  <mat-button-toggle *ngFor="let num of anzahlOptions" [value]="num">{{ num }}</mat-button-toggle>
...
onToggleChange($event: MatButtonToggleChange, name: String) {
  if (name.toLowerCase().includes("kinder")) {
    ...
    let newList = this.personenListe.filter(p => !p.Art.name.toLowerCase().includes(name.toLowerCase() + " enkel"));
    
    for (let i = 0; i < $event.value; i++) {
      const personType = new PersonType("Enkel", `${name + " Enkel"} ${i+1}`);
  ...
  if (name.toLocaleLowerCase().includes("geschwister")) {
    this.personenListe = this.personenListe.filter(p => !p.Art.name.toLocaleLowerCase().includes(name.toLocaleLowerCase() + " cousin"));

    console.log(this.personenListe)

    for (let i = 0; i < $event.value; i++) {
      const personType = new PersonType("Enkel", `${name + " Cousin"} ${i+1}`);
      const person = new Persen(personType);
...
get filteredPersonenListe() {
  return this.personenListe.filter(p => 
    p.Art.name && (
