...
public verwandtschaftsverhaeltnisse: FormGroup;
  ...
  private setupFormListeners(): void {
    ...
    this.verwandtschaftsverhaeltnisse.get('ehepartner')?.valueChanges.subscribe(checked => {
      if (checked) {
    ...
    this.verwandtschaftsverhaeltnisse.get('vater')?.valueChanges.subscribe(checked => {
      if (checked) {
    ...
    this.verwandtschaftsverhaeltnisse.get('mutter')?.valueChanges.subscribe(checked => {
      if (checked) {
    ...
    this.verwandtschaftsverhaeltnisse.get('kinder')?.valueChanges.subscribe(checked => {
      if (checked) {
    ...
    this.verwandtschaftsverhaeltnisse.get('geschwister')?.valueChanges.subscribe(checked => {
      if (checked) {
        const anzahl = this.verwandtschaftsverhaeltnisse.get('geschwisterAnzahl')?.value || 1;
    ...
    this.verwandtschaftsverhaeltnisse.get('geschwisterAnzahl')?.valueChanges.subscribe(anzahl => {
    ...
    this.verwandtschaftsverhaeltnisse.get('nachkommen')?.valueChanges.subscribe(checked => {
      if (checked) {
        const anzahl = this.verwandtschaftsverhaeltnisse.get('nachkommenAnzahl')?.value || 1;
    ...
    this.verwandtschaftsverhaeltnisse.get('nachkommenAnzahl')?.valueChanges.subscribe(anzahl => {
  ...
  private addPerson(personenArt: "Ehegatte" | "Lebensgefährte" | "Mutter" | "Vater" | "Geschwister" | "Kinder"): void {
  ...
  private removePersonByType(personenArt: string): void {
    ...
    this.personenListe = this.personenListe.filter(p => p.Art.PersonenArt !== personenArt);
    console.log(`All persons with type ${personenArt} removed`);
    console.log('Current list:', this.personenListe);
  }
  
  public addMultiplePersons(personenArt: "Ehegatte" | "Lebensgefährte" | "Mutter" | "Geschwister" | "Vater" | "Kinder", anzahl: number): void {
...
export class PersonType {
  PersonenArt: "Ehegatte" | "Lebensgefährte" | "Mutter" | "Vater" | "Geschwister" | "Enkel" | "Cousin" | "Kinder";
  name: string;

  constructor(personenArt: "Ehegatte" | "Lebensgefährte" | "Mutter" | "Vater" | "Geschwister" | "Kinder" | "Enkel" | "Cousin" = "Ehegatte", name: string = "") {
